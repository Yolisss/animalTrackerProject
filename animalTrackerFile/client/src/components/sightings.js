...
const Sightings = () => {
  ...
  const handleDeleteSighting = async (deleteId) => {
    const response = await fetch(
      `http://localhost:8080/sightings/${deleteId}`,
      {
        method: "DELETE",
      }
    );
    await response.json();
    const deleteSightingFunction = sightings.filter(
          ...
          {sightings
            .filter((val) => {
              if (searchTerm === "") {
                ...
                val.date_time.toString().includes(searchTerm.toString())
              ) {
                return val;
              } else if (
                ...
                val.individual_id.toString().includes(searchTerm.toString())
              ) {
                return val;
              } else if (
                val.created_on.toString().includes(searchTerm.toLowerCase())
              ) {
                return val;
              } else if (
                val.healthy
